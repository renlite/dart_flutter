...
The [RenderView](https://api.flutter.dev/flutter/rendering/RenderView-class.html) kicks off the walk down the RenderTree. As [RenderView](https://api.flutter.dev/flutter/rendering/RenderView-class.html) is a container RenderObject which can hold one child, it has a mixed in type [RenderObjectWithChildMixin<RenderBox>](https://api.flutter.dev/flutter/rendering/RenderObjectWithChildMixin-mixin.html). With the assignement `RenderingFlutterBinding flutterBinding = RenderingFlutterBinding(root: RenderStack( ...` the child of [RenderView](https://api.flutter.dev/flutter/rendering/RenderView-class.html) is set to [RenderDecoratedBox](https://api.flutter.dev/flutter/rendering/RenderDecoratedBox-class.html) and the child setter of RenderView is called. Then `adoptChild(_child)` is invoked which is implemented in the [AbstractNode](https://api.flutter.dev/flutter/foundation/AbstractNode-class.html).
...
```
Here the [RenderView](https://api.flutter.dev/flutter/rendering/RenderView-class.html) is assigned to be the child's parent and `child.attach(_owner!);` method (defined in [RenderObjectWithChildMixin](https://api.flutter.dev/flutter/rendering/RenderBox-class.html)) is called, where `super.attach(owner);` of [RenderObject](https://api.flutter.dev/flutter/rendering/RenderObject-class.html) and also `super.attach(owner);` of [AbstractNode]https://api.flutter.dev/flutter/foundation/AbstractNode-class.html is invoked. In AbstractNode's attach method the [PipelineOwner](https://api.flutter.dev/flutter/rendering/PipelineOwner-class.html) is finally set to the child's (RenderDecoratedBox) `_owner` property and the RenderObject's attach method marks the child as dirty. After all if the child ([RenderDecoratedBox](https://api.flutter.dev/flutter/rendering/RenderDecoratedBox-class.html)) itself has a child (RenderSizedOverflowBox), the [RenderSizedOverflowBox](https://api.flutter.dev/flutter/rendering/RenderSizedOverflowBox-class.html)' attach method will be called and so on, till a leaf RenderObject has no child. 
...
```
That means you can create more RenderTrees by composition, but only that one that is appended to the [RenderView](https://api.flutter.dev/flutter/rendering/RenderView-class.html) or to a child of the [RenderView](https://api.flutter.dev/flutter/rendering/RenderView-class.html) is attached and painted to the screen. When a root [RenderObject](https://api.flutter.dev/flutter/rendering/RenderObject-class.html) of a RenderTree is appended to another [RenderObject](https://api.flutter.dev/flutter/rendering/RenderObject-class.html) that is already attached, the attach method of all RenderObjects of the RenderTree is called. On the other side when an attached [RenderObject](https://api.flutter.dev/flutter/rendering/RenderObject-class.html) is removed (child = null;) from parent's child the detach method is called for the removed [RenderObject](https://api.flutter.dev/flutter/rendering/RenderObject-class.html) and all it's children down the tree. For example, there are tree RenderTrees Page_A, Page_B and Page_C. We could prepare all tree Pages by composition and assign only Page_A to the [RenderView](https://api.flutter.dev/flutter/rendering/RenderView-class.html). Page_A will be rendered. Later on an user action we change the assigntment from Page_A to Page_B. All members of Page_A will be detached and all memebers of Page_B will be attached. Page_B will be showed. If we still hold a reference to the root [RednerObject](https://api.flutter.dev/flutter/rendering/RenderObject-class.html) of Page_A the RenderTree of Page_A is available for later attachment without the need to (re)create new RenderObjects. The programmer can decide how to manage (re)creation of [Dart's](https://dart.dev/) objects and memory. In a typical [Flutter](https://flutter.dev/) app this is automatically managed by the framework in the ElementTree. Direct handling of RenderTree offers more control.

### Key of RenderObject
Unfortunatelly, it is not easy to find a special RenderObject in the RenderTree by a Key like in the HTML-DOM e.g. to change the RenderObject or simply to change the color property. This is not necessery for Widget based apps because the rebuilding of a view is done by the ElementTree. An Element holds the positon of a Widget in the RenderTree and is responsible for the complicated diffing between Widgets. Theoretically it is possible to find a special RenderObject by walking up or down the RenderTree but this is not efficient and in most cases not recomended.
Amongst others Renlite brings the missing Key to RenderObjects with a very small framework '[service.dart](https://github.com/renlite/dart_flutter/blob/master/renlite/lib/src/foundation/service.dart)'. RenliteKeyMixin uses the attach method to register a Key to an attached RenderObject. A very lightweight wrapper over a RenderObject using the [ReliteKeyMixin](https://github.com/renlite/dart_flutter/blob/master/renlite/lib/src/foundation/service.dart#L44) makes it possible to find ther UI object later. For a better understanding and distinction I use the name convention Renlite* instead of Render*, e.g. [RenliteParagraph](https://github.com/renlite/dart_flutter/blob/master/renlite_objects/lib/src/rendering/paragraph.dart) ([RenderParagraph](https://api.flutter.dev/flutter/rendering/RenderParagraph-class.html)), [RenliteTransform](https://github.com/renlite/dart_flutter/blob/master/renlite_objects/lib/src/rendering/transform.dart) ([RenderTransform](https://api.flutter.dev/flutter/rendering/RenderTransform-class.html)) or [RenliteSimpleButton](https://github.com/renlite/dart_flutter/blob/master/renlite_objects/lib/src/rendering/button.dart) (Composition of RenderObjects ).  
```Dart
import 'package:flutter/rendering.dart';
